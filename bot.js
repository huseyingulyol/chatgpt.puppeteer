...
(async () => {
  ...
  const browser = await puppeteer.launch({ headless: false,
    ...
    args: [
      '--user-data-dir=C:\\Users\\strea\\AppData\\Local\\Google\\Chrome\\User Data', // Profil dizininin yolu
  ...
  await pagePercipio.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36');
  ...
  browser.on('targetcreated', async (target) => {
    if (target.type() === 'page') { // Hedef bir sayfa mı kontrol edin
            ...
            while (isStarted)
            {
              //SORUYU AL
                ...
                async function SolveQuestion()
                {
                  ...
                  const question = `
                      ...
                      Cevap şıkları:
                      ${ 
                      Array.from(document.querySelectorAll('button[data-testid*="multiple-choice-answer"]'))
              ...
              let answer = await pageChatGPT.evaluate(async (message) => {
                ...
                async function GetAnswer()
                {
                  ...
                  let answerChatGPT = document.querySelectorAll("div[data-message-author-role='assistant']")[document.querySelectorAll("div[data-message-author-role='assistant']").length - 1].textContent;
              ...
              await newPage.evaluate(async (message) => {
                ...
                {
                  let currentCounter = document.querySelector("span[aria-label*='Question']").textContent;
                  console.log("Counter'a erişildi!");
                  console.log(currentCounter);

                  console.log("Gelen cevap:", message);
                  await checkElementAndClick(`button[data-testid*="${message.trim()}"]`);
                  console.log("cevaba tıklandı!");
                  await delay(1000);
                  await checkElementAndClick('button[data-testid*="quiz-button-cta"]');
                  await delay(300);
                  ...
                  await delay(200);
                  while (!(await checkElement("span[aria-label*='Question']")))
                  {
                    await delay(200);   
                  ...
                  while (currentCounter == document.querySelector("span[aria-label*='Question']").textContent)
                  {
